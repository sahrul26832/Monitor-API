$(document).ready(function () {
  ...
  loadRequests(function (data) {
    ...
    // Register filter after table init
    registerRequestFilters();
  });

  // ===== Resend Functions (API-based) =====
  ...
  $(document).on('click', '.btn-resend', function () {
    ...
    console.log('Resend clicked for ID:', reqId);
    var reqObj = requestData.find(function (r) { return r.id === reqId; });
    if (!reqObj) {
      console.warn('Request data not found in local cache, refreshing...');
      refreshTables();
      return;
    }
    showModal(reqObj);
  ...
  // ===== Modal Confirm: call API to resend =====
  ...
  // ===== Ignore Button Handler: call API =====
  $(document).on('click', '.btn-ignore', function () {
    var reqId = $(this).data('request-id');
    console.log('Ignore clicked for ID:', reqId);
    ...
    $.ajax({
      ...
      contentType: 'application/json',
      success: function (result) {
        console.log('Ignore success:', result);
        $row.fadeOut(300, function () { $(this).remove(); });
        showNotification(result.message, 'success');
      },
      error: function (xhr) {
        console.error('Ignore error:', xhr);
        showNotification('Ignore ไม่สำเร็จ: ' + (xhr.responseJSON?.error?.message || 'Unknown error'), 'error');
      }
    });
  });
  ...
  // ===== Request Monitor Filter Controls =====
